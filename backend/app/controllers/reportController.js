...
exports.getRevenueReportByRoomType = async (req, res) => {
  try {
    ...
    if (!year || !month) {
      return res
        .status(400)
        .json({ message: "Year and month parameters are required" });
    }
    ...
    if (totalRevenue > 0) {
      Object.keys(revenueByType).forEach((type) => {
        revenueByType[type].percentage =
          Math.round((revenueByType[type].revenue / totalRevenue) * 100 * 10) /
          10;
      });
    }
...
exports.getOccupancyReportByRoomType = async (req, res) => {
  try {
    ...
    for (const room of rooms) {
      ...
      invoices.forEach((invoice) => {
        const roomRental = invoice.rentals.find(
          (rental) => rental.roomNumber === room.roomNumber
        );
    ...
    if (totalDaysRented > 0) {
      occupancyData.forEach((item) => {
        item.occupancyRate =
          Math.round((item.daysRented / totalDaysRented) * 100 * 10) / 10;
      });
    }
